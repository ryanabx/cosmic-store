...
impl RpmOstree {
        ...
        if parts.len() >= 2 {
            ...
            for line in staged_section.lines() {
                ...
                if line.starts_with("BaseCommit:") {
                    if let Some(version) = line.split(':').nth(1).map(|s| s.trim().to_string()) {
    ...
    fn parse_upgrade_check(
        ...
        // If the available update matches the staged (already-queued) version by commit, skip it
        ...
        Ok(vec![Package {
            id: AppId::system(),
            icon: widget::icon::from_name("system-upgrade").size(128).handle(),
            info: Arc::new(AppInfo {
                source_id: "rpm-ostree".to_string(),
                source_name: "rpm-ostree".to_string(),
                name: "System Update".to_string(),
                summary: format!("Update to version {}", commit),
                description: "System upgrade available".to_string(),
